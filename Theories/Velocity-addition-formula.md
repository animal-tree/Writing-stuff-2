...
- Note: This implicitly assumes that *at least* two of the body $\mathrm{B}$'s positions that happen chronologically in one reference frame $K^{(i)}$ can be translated across all other (considered) reference frames $K^{(j)}$ so as to happen in the same chronological order in all other (considered) reference frames $K^{(j)}$. 
