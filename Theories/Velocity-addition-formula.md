...
### (2) Measurements and displacements.
...
&= \mathcal{V}(v^{(i, j)}, \frac{\partial x_{t^{(i)}}^{(i)}}{\partial t^{(i)}}). \quad\quad \text{(Definition 4)} \quad\quad\quad \square
\end{align}
$$
