...
But Newton's 3rd law is violated by photons when it comes to their interaction with gravity. Gravity causes photons to "react" via centripetal acceleration towards masses, but the equal and opposite reaction isn't present, meaning $\frac{\partial F_\gamma}{\partial t}$ $\neq$ $-\frac{\partial F_{e^{-}}}{\partial t}$, where $F_\gamma$ is the (e.g., gravitational) force on the photon by the electron, and $F_{e^{-}}$ is the force on the electron by the photon. Quick proof, since this requires reconsidering the ordinary definition of a photon's momentum: The force of a photon, $F_\gamma$ $=$ $\frac{\partial p}{\partial t}$ $=$ $\frac{\partial \frac{hf}{c}}{\partial t}$ is ordinarily thought to be $0$, but momentum has direction, including a photon's momentum, so this value $\frac{\partial p}{\partial t}$ indeed is not $0$ when the force of gravity bending it is applied, where $h$ is Planck's constant and $f$ is the frequency of the photon, $\neq$ the negative force on the electron, $-F_{e^{-}}$ $=$ $0$.
