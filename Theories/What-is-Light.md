...
- Ah, here's one possible critique that can be made: the accelerations (forces) from conflicting polarizations don't interfere because both act in the direction of velocity. However, that's only because of the electric field, which dpeends on polarization and is interfered with, and the magnetic field, which also depends on polarization and is interfered with, so no. The electromagnetic "momentum" is cancelled out in unpolarized light, and there would be no physical trace of it by which to see it.
- An alternative critique is that those fields don't interfere with each other and two superimposed photons with opposite polarizations somehow independently affect the electron, but then there's the question of "in which order?" do they impart their two independent effects, given that the probability distributions would cancel out (so the wave function can't be fallen back on as an explanation) and those electric and magnetic vibrations are conflicting, so by Newton's 2nd law, the forces have to cancel out.
