...
$`D(b_\mathrm{K},b_\mathrm{t}, x) = \sum\limits_{q = 1}^{N} \mathrm{p_\omega}^{(b_\mathrm{K},b_\mathrm{t},q)}(x)`$,
...
$`\mathrm{\hat{h}_a}^{(K,t,i,j,k)} = - \nabla_{\mathrm{h_x}^{(K,t-1,i,j)}} D(K,t-1, \mathrm{h_x}^{(K,t-1,i,j)})`$,
...
> [!NOTE]
> In only the usage immediately above, $`\mathrm{\hat{h}}_{\mathrm{p}^{(t)}}^{(K,t,i,j,k)}`$ doesn't yet belong to the $`\mathrm{\hat{h}}^{(K,t,i,j,k)}`$ element of $`\mathcal{G}^{(K, t, i)}`$, since $`\mathrm{\hat{h}}_{\mathrm{p}^{(t)}}^{(K,t,i,j,k)}`$ isn't yet defined. All necessary elements for this usage of $`\mathrm{\hat{h}}^{(K,t,i,j,k)}`$ of $`\mathcal{G}^{(K, t, i)}`$ are defined.
