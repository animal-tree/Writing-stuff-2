...
When $r$ increases, $v$ must decrease. However, somehow, when $\frac{n}{r}$ increases, $v$ increases, despite the fact that $n$ must increase proportionally to $r$. How is that possible? Only if $n$ is a function of $r$, say $n = f(r)$, such that: 
