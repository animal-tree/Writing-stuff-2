...
Deduced, not proven. Either Einstein's first postulate (notated without loss of generality here as $x_{t^{(i)}}^{(i)} = ct^{(i)} \text{ } \forall i$, or above as "$x = ct$ and $x' = ct'$"), or the formula's usefulness, is false. One or the other has to go.
