...
The origin of any reference frame $K^{(i)}$ is conformed with wherever $x_{0}^{(i)}$ is. A reference frame $K^{(i)}$ can be defined or redefined such that $x_{0}^{(i)} = 0$, if not already the case, by means of a fixed translation in space of the measuring system. Such a re-centering has no effect on relative velocities $v^{(i,j)}$ or $v^{(j,i)}$ between observers and bodies on reference frames. Alternatively, if this isn't satisfying, $x_{0}^{(i)} = 0$ may also be derived directly as a necessary consequence of the constancy of light postulate, which states that $x_{t^{(i)}}^{(i)} = ct^{(i)}$: 
